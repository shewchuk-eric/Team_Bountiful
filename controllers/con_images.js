...
const listByBook  = async (req, res) => {
  ...
  */
  ...
  try {  
    ...
    const result = await mongodb.getDb().db('team_bountiful').collection('images').find({ bookWhereSeen: bookParam });
...
const listByCharacter  = async (req, res) => {
  ...
  */
  ...
  try {  
    ...
    const result = await mongodb.getDb().db('team_bountiful').collection('images').find({ characterName: characterParam });
...
const updateImage  = async (req, res) => {
  ...
  */
  ...
  try {  
    ...
    const response = await mongodb.getDb().db('team_bountiful').collection('images').replaceOne({ _id: imageId }, image);
    if (response.modifiedCount > 0) {
      res.status(204).send();
    } else {
      return res.status(404).json({ messsage: 'The image with the specified id was not found or there was not a change in the request body.' });
    }
  } catch (error) {
    return res.status(500).json({
      message: error.message || 'An internal server error occurred.'
  ...
  */
  ...
  try {  
    ...
    if (response.deletedCount === 1) {
      ...
      return res.status(404).json({ messsage: 'The  with the specified id was not found.' });
    }
  } catch (error) {
    return res.status(500).json({
      message: error.message || 'An internal server error occurred.'
